...
touch password.txt
echo "cytomine:$RETRIEVAL_PASSWORD" > password.txt
...
if [ "$ENGINE" == "memory" ] 
then
	java -jar retrieval-0.5-SNAPSHOT.war --spring.profiles.active=prod \
	    --retrieval.store.name=MEMORY \
	    --retrieval.thumb.index=/data/thumb/index \
	    --retrieval.thumb.search=/data/thumb/search > /tmp/retrieval.log
else
	cd /data/thumb && redis-server /tmp/redis.conf&

	java -jar retrieval-0.5-SNAPSHOT.war --spring.profiles.active=prod \
	--retrieval.store.name=REDIS \
	--retrieval.thumb.index=/data/thumb/index \
	--retrieval.thumb.search=/data/thumb/search > /tmp/retrieval.log
fi
